module.exports = {
  ...
  create(context) {
    const [
      checkMemberNameStartByUpperCase = true,
      checkMemberNamePascal = true,
    ] = context.options
    return {
      TSEnumDeclaration({ id, members }) {
        ...
        if (checkMemberNameStartByUpperCase || checkMemberNamePascal) {
          members.forEach((member) => {
            if (
              checkMemberNameStartByUpperCase &&
              /^[a-z]/.test(member.id.name)
            ) {
              context.report({
                node: member.id,
                ...
                fix: (fixer) => {
                  return fixer.replaceText(
                    member.id,
                    `${member.id.name.charAt(0).toUpperCase()}${member.id.name.slice(1)}`
                  )
                },
              })
            } else if (
              checkMemberNamePascal &&
              /^[A-Z0-9_]+$/.test(member.id.name)
            ) {
              context.report({
                node: member.id,
                ...
                fix: (fixer) => {
                  return fixer.replaceText(
                    member.id,
                    `${member.id.name.charAt(0)}${member.id.name
                      .slice(1)
                      .toLowerCase()
